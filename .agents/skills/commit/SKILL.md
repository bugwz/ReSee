---
name: commit
description: 为“回见”iOS 项目检查全部 Git 变更，生成英文 Conventional Commit 提交信息，并在用户使用中文或英文明确确认后执行提交。用户要求提交代码、创建 commit、整理提交信息或准备版本提交时使用。
---

# 回见项目提交

为当前仓库准备边界清晰、可复核的 Git 提交。提交信息使用英文；分析默认使用中文，用户可使用中文或英文确认。
...
确认提交吗？请回复“确认”或 `yes` 继续，回复“取消”或 `no` 终止。
````

中文和英文确认具有同等效力，不得要求用户改用特定语言。确认词包括“确认”“是”“提交”以及 `yes`、`y`、`confirm`；拒绝词包括“取消”“否”以及 `no`、`n`、`cancel`，匹配时忽略大小写并允许常见结尾标点。其他回复均视为不明确，回复：

> 您的回复不明确。请回复“确认”或 `yes` 继续提交，回复“取消”或 `no` 终止。
