...
“回见”（工程名 `ReSee`）是一款原生 iOS 空间记录应用。产品目标不是固定视点的 360° 全景，而是通过 ARKit 采集相机位姿、深度和空间网格，形成可在有限采集范围内移动查看、缩放和标注的 6DoF 数字空间。

当前仓库是 MVP 骨架，优先闭环“场景列表 → AR 采集 → 本地保存 → 查看与标注”。现阶段只保存场景索引和采集统计，尚未持久化相机帧、深度、网格或 USDZ，也尚未接入摄影测量和 3DGS。实现和文档不得把规划能力描述成已经完成。
