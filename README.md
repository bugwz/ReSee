...
- 定点全景：在一个位置按 5 层俯仰带采集 52 个球面方向，生成一张标准 2:1 等距柱状全景图
- 多点全景：记录 3 个间距至少 1 米的固定点，每个点独立生成完整 360° 全景图
- ARKit / RealityKit 采集界面与自动完成状态机
- LiDAR 场景重建能力检测、深度语义和网格可视化
- 当前点位坐标、跟踪状态、球面覆盖图、遗漏方向和移动距离引导
- 每个方向需满足连续位姿稳定与 12 cm 光心漂移约束；应用从两张 ARKit 高分辨率候选帧中按拉普拉斯清晰度择优，降低手持抖动和运动模糊
- 覆盖完整后自动结束；Metal 在线性色彩空间执行清晰度与抗视差优先的两阶段选帧融合，并对偏离稳健光心的帧降权，生成 6144 × 3072 HEIC 全景图和场景元数据
...
当前索引采用新的全景数据结构，要求记录方式、采集统计和全景相对路径等字段完整存在，不迁移也不兼容旧八方向记录。空间标注不只保存一次 AR 会话的世界坐标，还预留了模型版本、法线、三角形编号和重心坐标：
