...
> 项目目前处于 MVP 阶段。应用内采集仍以定点和三点 360° 全景为主；外部资源浏览已支持 3D Gaussian Splatting（3DGS），但应用内训练与重建仍是后续能力。
...
- 外部资源库：可从网络下载，或从系统文件选择器载入全景与 3DGS 文件；本地文件可选择复制到应用目录或保留安全引用
- 外部定点/多点全景支持 JPG、JPEG、PNG、HEIC、HEIF；多选图片会作为离散点位浏览
- 外部 3DGS 支持 Gaussian PLY、SPLAT 与 SPZ，使用 MetalSplatter 原生 Metal 渲染，并提供移动、朝向和俯仰双摇杆
...
- 开启 iCloud 后，可分别选择同步生成全景、外部资源信息和应用内外部数据文件；外部引用只同步信息，不会越权上传原位置文件。原始相机帧、深度和网格仍不会上传
- 相机权限声明和不支持设备的降级提示
...
多点模式是在三个离散固定点之间切换，并不支持点位之间连续 6DoF 移动。外部 3DGS 文件可 6DoF 浏览，但应用尚不能从自身采集数据训练 3DGS。应用会持久化最终全景图，但还不会持久化完整原始相机帧流、深度、网格或 USDZ。
...
外部 3DGS 浏览固定使用 [MetalSplatter 1.0.1](https://github.com/scier/MetalSplatter)（MIT），并通过 Swift Package Manager 锁定版本；其传递依赖 `spz-swift` 为 MIT，`swift-argument-parser` 为 Apache-2.0。后续引入重建组件时仍需审查全部传递依赖：COLMAP（BSD-3-Clause）、Open3D（MIT）通常较易用于商业项目；OpenMVS（AGPL-3.0）、ORB-SLAM3（GPL-3.0）以及部分原始 3DGS 实现需要额外评估，不应默认可直接集成到闭源 App。
