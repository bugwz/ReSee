...
- 定点全景：竖直握持手机，将画面中的动态目标点依次移入中央对准环；在一个位置按 5 层俯仰带采集 52 个球面方向，生成一张标准 2:1 等距柱状全景图
...
- 每个方向需满足连续位姿稳定与 12 cm 光心漂移约束；稳定后只请求一张 ARKit 高分辨率帧，减少连续采集时的停顿
- 横持手机时暂停采集；高低俯仰带覆盖到 ±75°，减少天顶和地面黑边。覆盖完整后自动结束；Metal 在线性色彩空间执行清晰度与抗视差优先的两阶段选帧融合，并对偏离稳健光心的帧降权，生成 6144 × 3072 HEIC 全景图和场景元数据
